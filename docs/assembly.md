...
The Arduino controls the syringe pumps, stirrer, and the temperature control pumps, while the Raspberry Pi follows the recipe that it has been ordered to execute, monitors the temperature of the Lab Reactor, and the reaction time. 
...
Download the BOM here: [https://github.com/FourThievesVinegar/solderless-microlab//tree/main/BOM](https://github.com/FourThievesVinegar/solderless-microlab/tree/master/BOM)


### 3D Printed Parts
While most items on the parts list can be purchased, two items will need to be printed using a 3D printer: syringe pumps and reactor manifold. 
...
1. Arduino Setup 
	...
	1. Note: Video overview of CNC shield assembly and operation
[https://youtu.be/zUb8tiFCwmk?t=37](https://youtu.be/zUb8tiFCwmk?t=37)
	1. Note: Critical safety information when working with the CNC shields. Failure to follow them can hurt both you and the componeNEVER connect or disconnect any stepper motor to the CNC Shield while the power is on or connected
