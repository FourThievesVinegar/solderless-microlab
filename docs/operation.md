 ...
 * User Option(s) - next steps user can take after task is done (used with Human Tasks)
...
* Stir - turn on stir rod for a set period of time with optional task to maintain temperature
...
* Step 10: Human task - Confirmation that user tapped no to doing anothr test and test is complete
...
New recipes can be made online, downloaded to a thumbdrive and then to the Control Unit.  
