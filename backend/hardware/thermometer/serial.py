...
class SerialTempSensor(TempSensor):
    ...
    def read_sensor(self) -> str:
        ...
        while True:
            try:
                reading = self.tempSer.readline().decode('utf-8', errors='ignore')
            except Exception as e:
                self._logger.error(t['error-reading-thermometer'])
                self._logger.exception(str(e))
            else:
                self._logger.debug(t['ser-read'].format(str(len(reading)), reading))
    ...
    def getTemp(self) -> float:
        ...
        self._logger.debug(
            t['thermometer-found'].format(str(start), str(end), sensor_reading, sensor_reading[start:end])
        )
        ...
        if start > -1 and end > -1 and end - start > 2:
            try:
                ...
                self._logger.error(sensor_reading[start:end])
                self._logger.exception(str(e))
                self.lastTemp = -999.0
        else:
            self.lastTemp = -999.0
            self._logger.error(t['error-reading-thermometer-specific'].format(sensor_reading[start:end]))
        self._logger.debug(
